...
end_date = '12/31/2022'
LIS_res = 0.1 # ~1 km resolution
REGION = 'gambia'
FOLDER = 'imerg_precipitation'
...
for index, row in wells_df.iterrows():
    ...
    for type_num in climateserv_datatypes:
        ...
        df = ca.getCSV(type_num, start_date, end_date, "Average", ca.getBox(lat=row['Latitude'], lon=row['Longitude'], res=LIS_res), f"data/original_data/climateserv/{FOLDER}/{REGION}/{row['ID']}.csv")
